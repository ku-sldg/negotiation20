...
This file includes the module `DepCopland`. In this module, there is a Privacy Policy that maps evidence to [True] or [False]. It returns a `Prop` and is accomplished with a Fixpoint function. There is also a function `privPolicyT` that allows the pricacy policy to be defined over terms rather than evidence.  
